...
- **[CORRIGIDO na implantação do Compras Diversos, 07/2026] Papéis por grupo exigem `GroupMember.Read.All` + `User.Read.All` (Application).** Sem elas, o `transitiveMemberOf` devolve `Insufficient privileges` e o `getUserRoles` engole o erro devolvendo lista vazia — os perfis por grupo somem SILENCIOSAMENTE e o sintoma é o usuário "sem perfil de acesso" mesmo estando nos grupos certos. Diagnóstico rápido: abrir `/api/MeusGrupos` logado (o endpoint expõe o erro real). Conceder as duas permissões com admin consent resolve em ~5 min (cache de grupos do backend).
