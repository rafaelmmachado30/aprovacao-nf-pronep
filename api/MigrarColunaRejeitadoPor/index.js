/**
 * /api/MigrarColunaRejeitadoPor
 *
 * Adiciona 2 colunas em PRONEP-NF-NotasFiscais para registrar QUEM rejeitou/estornou
 * (antes o front mostrava por engano o APROVADOR na coluna "Rejeitado por"):
 *   - RejeitadoPor (text) - e-mail de quem rejeitou/estornou
 *   - RejeitadoEm  (text) - data/hora ISO da rejeicao
 *
 * Idempotente (nao recria se ja existe). RBAC: admin only.
 */
...
const COLS = [
  { name: 'RejeitadoPor', def: { text: {} } },
  { name: 'RejeitadoEm',  def: { text: {} } }
];
...
module.exports = async function (context, req) {
  ...
  try {
    ...
    for (const c of (colsResp.value || [])) { if (c.displayName) existentes.add(c.displayName); if (c.name) existentes.add(c.name); }

    for (const col of COLS) {
      if (existentes.has(col.name)) { diag.jaExistia.push(col.name); continue; }
      try {
        await client.api('/sites/' + siteId + '/lists/' + listId + '/columns').post(Object.assign({ name: col.name }, col.def));
    ...
    diag.step = 'done';
    context.res = { status: 200, headers: { 'Content-Type': 'application/json' }, body: Object.assign({ ok: diag.falhas.length === 0 }, diag) };
  } catch (err) {
