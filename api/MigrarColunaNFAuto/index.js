/**
 * /api/MigrarColunaNFAuto
 *
 * Adiciona coluna `NumeroAutogerado` (boolean Yes/No) na lista PRONEP-NF-NotasFiscais,
 * necessaria pro fluxo de auto-numeracao quando o usuario submete NF sem numero
 * (reembolsos, rescisoes, etc).
 *
...
const COL_NAME = 'NumeroAutogerado';
  ...
  try {
    ...
    if (!diag.colunaJaExistia) {
      ...
      try {
        ...
        // Tenta como text como fallback (boolean as vezes nao aceito)
        try {
          diag.step = 'create_column_fallback_text';
          await client.api('/sites/' + siteId + '/lists/' + listId + '/columns').post({
            name: COL_NAME,
            text: {}
          });
          diag.colunaCriada = true;
          diag.fallbackUsado = 'text';
        } catch (eText) {
          diag.erros.push({
            step: 'create_column_fallback_text',
            error: eText.message,
            graphCode: eText.code,
            graphStatusCode: eText.statusCode,
            graphBody: eText.body
          });
        }
      }
    }
